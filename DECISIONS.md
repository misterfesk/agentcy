...
- **Inference provider:** Agentcy subagents use Nebius Token Factory's OpenAI-compatible API (`https://api.tokenfactory.nebius.com/v1/`) with `zai-org/GLM-5.3-Flash`, verified against the authenticated Nebius model list. The Nebius API key is read from a private secret file, never from source or a container environment.
