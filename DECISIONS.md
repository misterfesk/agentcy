...
- **Inference provider:** Agentcy subagents use Nebius Token Factory's OpenAI-compatible API (`https://api.tokenfactory.nebius.com/v1/`) with the user-selected model identifier `glm5.3flash`. The Nebius API key is read from a private secret file, never from source or a container environment. Before live activation, a startup/preflight check must validate this literal model identifier against Nebius's authenticated `/v1/models` response; the public documentation does not confirm that identifier.
